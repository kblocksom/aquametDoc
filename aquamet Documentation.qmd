...
chanMorphMets <- nrsaChannelMorphology(
  ...
  wDepth = wthalDepth |> 
    mutate(VALUE=as.numeric(VALUE)) |> 
    select(SITE, TRANSECT, STATION, VALUE, UNITS),
  wIncisedHeight =subset(nrsaChannel, SAMPLE_TYPE=='BANKW' & PARAMETER %in% c('INCISED'), UNITS='M') |>
    mutate(VALUE=as.numeric(VALUE)) |>
    select(SITE, TRANSECT, VALUE, UNITS),
  wWettedWidth = subset(nrsaThalweg, SAMPLE_TYPE=='THALW' & PARAMETER %in% c('WETWIDTH'), UNITS='M') |>
    mutate(VALUE=as.numeric(VALUE), UNITS='M') |>
    select(SITE, TRANSECT, STATION, VALUE, UNITS)
  )
```
